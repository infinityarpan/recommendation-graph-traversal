...
class ApiBehaviorTests(unittest.TestCase):
    def setUp(self):
        self.initialize = patch("app.main.GraphDatabase_Connection.initialize")
        self.close = patch("app.main.GraphDatabase_Connection.close")
    ...
    def test_health_endpoint_returns_success(self):
        with patch("app.routes.get_health_status", return_value={"status": "ok"}):
    ...
    def test_similar_endpoint_returns_recommendations(self):
        ...
        with patch("app.routes.get_similar_recommendations", return_value=payload):
    ...
    def test_runtime_errors_translate_to_503(self):
        with patch("app.routes.get_trending_recommendations", side_effect=RuntimeError("db down")):
    ...
    def test_missing_user_stats_returns_404(self):
        with patch("app.routes.fetch_user_stats", return_value=None):
